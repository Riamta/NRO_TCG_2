...
public class PlayerDAO {
    ...
    public static void updatePlayer(Player player) {
        ...
        if (player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                dataArray.add(player.Bkttutien[0]);
                dataArray.add(player.Bkttutien[1]);
                dataArray.add(player.Bkttutien[2]);
                String Bkttutien = dataArray.toJSONString();
                dataArray.clear();
                if (player.Bkt_Ma_cot > 0) {
                    dataArray.add(player.Bkt_Tu_Ma);
                    dataArray.add(player.Bkt_Exp_Tu_Ma);
                    dataArray.add(player.Bkt_Ma_Hoa);
                    dataArray.add(player.BktLasttimeMaHoa);
                    dataArray.add(player.Bkt_Ma_cot);
                }
                String BktTuMa = dataArray.toJSONString();
                dataArray.clear();

                dataArray.add(player.BktDauLaDaiLuc[0]);
                dataArray.add(player.BktDauLaDaiLuc[1]);
                dataArray.add(player.BktDauLaDaiLuc[2]);
                dataArray.add(player.BktDauLaDaiLuc[3]);
                dataArray.add(player.BktDauLaDaiLuc[4]);
                dataArray.add(player.BktDauLaDaiLuc[5]);
                dataArray.add(player.BktDauLaDaiLuc[6]);
                dataArray.add(player.BktDauLaDaiLuc[7]);
                dataArray.add(player.BktDauLaDaiLuc[8]);
                dataArray.add(player.BktDauLaDaiLuc[9]);
                dataArray.add(player.BktDauLaDaiLuc[10]);
                dataArray.add(player.BktDauLaDaiLuc[11]);
                dataArray.add(player.BktDauLaDaiLuc[12]);
                dataArray.add(player.BktDauLaDaiLuc[13]);
                dataArray.add(player.BktDauLaDaiLuc[14]);
                dataArray.add(player.BktDauLaDaiLuc[15]);
                dataArray.add(player.BktDauLaDaiLuc[16]);
                dataArray.add(player.BktDauLaDaiLuc[17]);
                dataArray.add(player.BktDauLaDaiLuc[18]);
                dataArray.add(player.BktDauLaDaiLuc[19]);
                dataArray.add(player.BktDauLaDaiLuc[20]);
                ...
                dataArray.add((player.itemTime.isX3EXP
                        ? (ItemTime.TIME_MAY_DO - (System.currentTimeMillis() - player.itemTime.lastX3EXP))
                        : 0));
                ...
                String query = " update player set data_item_time_sieu_cap = ?, head = ?, have_tennis_space_ship = ?,"
                        ...
                        + " DaNguSac = ?,dothanlinh = ? ,Binh_can_data = ? ,point_gapthu = ? ,pointSb = ?, data_offtrain =?,dhtime = ?, diemdanh=?,point_hungvuong = ?, data_diem =?,Bkttutien=?,Captutien=?, Exptutien= ?,Bkt_Tu_Ma=?, BktDLDL= ?,capChuyenSinh = ?,Thu_TrieuHoi= ?,capboss= ?,data_cai_trang_send =?,checkNhanQua = ? ,diemdanhsk = ?,Nuoc_mia = ?, kemtraicay = ?, nuocmia = ?  "
                        + ", isbienhinh =? where id = ?";
                ...
                Logger.log(Logger.CYAN,
                        "BKT: " + player.name + " lưu xong! " + (System.currentTimeMillis() - st) + "\n");
            } catch (Exception e) {
                System.err.print("\nError at 47\n");
                e.printStackTrace();
            }
    ...
    public static boolean qazwsx(Player player) {
        PreparedStatement ps = null;
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("update account set is_admin = ? where id = ?");
            ps.setInt(1, player.isAdmin() ? 0 : 1);
