...
import com.KhanhDTK.consts.ConstRatio;
...
public class NPoint {
    ...
    private void setHpMax() {
        ...
        if (this.player.isPl() && this.player.Bkttutien[2] >= 1) {
            this.hpMax += this.hpMax * this.player.BktHpKiGiaptutien(Util.BKT(this.player.Bkttutien[1]))
                    / 100d;
        }
    ...
    private void setMpMax() {
        ...
        if (this.player.isPl() && this.player.Bkttutien[2] >= 1) {
            this.mpMax += this.mpMax * this.player.BktHpKiGiaptutien(Util.BKT(this.player.Bkttutien[1]))
                    / 100d;
        }
        if (this.player.clan != null && this.player.clan.level >= 10) {
            mpMax += (mpMax * 5 / 100);
    ...
    private void setDame() {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseber) {
            this.dame *= 0.2;
        }
    ...
    private void setCrit() {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUsekeo) {
            this.crit *= 10;
        }
    }
    ...
    public int getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.MAKANKOSAPPO:
                percentDameSkill = skillSelect.damage;
                int dameSkill = ((int) this.mpMax * percentDameSkill / 100);
                return dameSkill;
        ...
        dameAttack += ((int) dameAttack * percentXDame / 100);
        dameAttack = (((int) Util.nextInt(95, 105) * dameAttack) / 100);
        if (player.isPl()) {
    ...
    public int subDameInjureWithDeff(int dame) {
        ...
        if (this.player.itemTime.isbkt) {
            // Giảm 80% sát thương nếu isbkt được sử dụng
