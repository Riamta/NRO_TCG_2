...
public class TimeUtil {
    ...
    public static String formatTimestamp(long timestamp, int type) {
        Instant instant = Instant.ofEpochMilli(timestamp);
        LocalDateTime localDateTime = instant.atZone(ZoneId.systemDefault()).toLocalDateTime();
        DateTimeFormatter formatter;

        if (type == 1) {
            formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy - H'h':m'm':s's'");
        } else {
            formatter = DateTimeFormatter.ofPattern("H'h':m'm':s's'");
        }
