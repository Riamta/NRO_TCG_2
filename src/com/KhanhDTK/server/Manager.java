...
import com.KhanhDTK.jdbc.daos.ShopDAO;
...
import com.KhanhDTK.models.intrinsic.Intrinsic;
import com.KhanhDTK.models.item.Item;
import com.KhanhDTK.models.map.WayPoint;
import com.KhanhDTK.models.map.gas.TopGas;
...
import com.KhanhDTK.models.reward.ItemOptionMobReward;
...
import com.KhanhDTK.models.task.SideTaskTemplate;
import com.KhanhDTK.models.task.SubTaskMain;
...
public class Manager {
    ...
    public static final List<com.KhanhDTK.models.map.Map> MAPS = new ArrayList<>();
    ...
    public static final String queryTopVuaHung = "SELECT id, CAST( point_hungvuong AS UNSIGNED) AS vua FROM player ORDER BY CAST( point_hungvuong AS UNSIGNED) DESC LIMIT 20";
    ...
    public static final String queryTopNap = "SELECT p.id, a.tongnap FROM player p JOIN account a ON p.account_id = a.id ORDER BY a.tongnap DESC LIMIT 20;";
...
// SKH VIP
    ...
    private void initMap() {
        ...
        for (MapTemplate mapTemp : MAP_TEMPLATES) {
            ...
            com.KhanhDTK.models.map.Map map = new com.KhanhDTK.models.map.Map(mapTemp.id,
    ...
    public static void loadPart() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            dos.flush();
            dos.close();
            Logger.log(Logger.BLACK, "/b|7|ngan tải dữ liệu part thành công binn (" + parts.size() + ")\n");
        } catch (Exception e) {
            System.err.print("\nError at 299\n");
            e.printStackTrace();
        }
...
//            Logger.log(Logger.BLACK, " tải dữ liệu Đại Hội Võ Thuật thành công (" + LIST_DHVT.size() + ")\n");
            ...
            Logger.log(Logger.BLACK, " tải dữ liệu mob template thành công (" + MOB_TEMPLATES.size() + ")\n");
...
//           // Thanh Tuu
            ...
            if (rs.first()) {
                ...
                Logger.log(Logger.BLACK, " tải dữ liệu map template thành công (" + MAP_TEMPLATES.length + ")\n");
            ...
            Logger.success("Load Top Vua Hùng Thành Công (" + TopVuaHung.size() + ")\n");
            ...
            Logger.success("Load top nhiệm vụ thành công (" + topNap.size() + ")\n");
            topNV = realTop(queryTopNV, con);
            Logger.success("Load top nap thành công (" + topNV.size() + ")\n");
        ...
        Logger.log(Logger.BLACK, "Tổng thời gian tải dữ liệu: " + (System.currentTimeMillis() - st) + "(ms)\n");
    }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    case queryTopSM:
                        if (pl != null) {
                            top.setInfo1("Tổng : " + (pl.nPoint.power + (pl.pet != null ? pl.pet.nPoint.power : 0))
                                    + "\nSM SP : " + pl.nPoint.power
                                    + "\nSM ĐT : " + pl.nPoint.power);
                        } else {
                            top.setInfo1("Tổng : " + Util.format(rs.getLong("sm") + rs.getLong("smd"))
...
//                        top.setInfo2("Update Time : " + TimeUtil.getTimeNow("HH:mm:ss") + "\n" + "|7|[ Ngọc Rồng Cooler ]");
                    ...
                    case queryTopGapThu:
                        ...
                        break;
                    case queryTopVuaHung:
                        top.setInfo1(rs.getInt("vua") + " điểm");
                        top.setInfo2(rs.getInt("vua") + " điểm");
                        break;
